...
class Simulation():
    def __init__(self):
        ...
        self.save_SSD = True #Save SSD figures
    ...
    def visualizeSSD(self, x_SSD_outer,y_SSD_outer,x_SSD_inner,y_SSD_inner):
        ''' VISUALIZING SSD'''
        print('************Visualise************')
        #Initialise the SSD variables and construct the SSD
        self.SSDVariables(traf.asas, traf.ntraf)
        self.constructSSD1(traf.asas, traf)

        #Start the visualisation and saving process

        for i in range(traf.ntraf):
            if  traf.asas.cr_name == "SEQSSD": #Delete so that all A/C are evaluated
